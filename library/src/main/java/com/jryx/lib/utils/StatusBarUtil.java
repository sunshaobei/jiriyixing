...
public class StatusBarUtil {
     ...
     * @param alpha
     */
    public static void init(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        immersive(activity.getWindow(), color, alpha);
    }

    private static void immersive(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float a) {
        float alpha = 1;
        if (isFlyme4Later()||isMIUI6Later()|| Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            alpha  = 0;
        } else {
            alpha = a;
        }
...
//            window.addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
    ...
    public static void darkMode(Activity activity, boolean dack) {
        darkMode(activity.getWindow(), DEFAULT_COLOR, DEFAULT_ALPHA,dack);
    }

    public static void darkMode(Activity activity, int color, @FloatRange(from = 0.0, to = 1.0) float alpha, boolean dack) {
        darkMode(activity.getWindow(), color, alpha,dack);
    }
    /** 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上) */
    public static void darkMode(Window window, int color, @FloatRange(from = 0.0, to = 1.0) float alpha, boolean dack) {
        if (isFlyme4Later()) {
            darkModeForFlyme4(window, dack);
            immersive(window,color,alpha);
        } else if (isMIUI6Later()) {
            darkModeForMIUI6(window, dack);
            immersive(window,color,alpha);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            darkModeForM(window, dack);
            immersive(window, color, alpha);
        } else if (Build.VERSION.SDK_INT >= 19) {
            immersive(window,color,alpha);
        } else {
    ...
    public static void initStatusBar(View v){
        if (Build.VERSION.SDK_INT >= 19) {
            ViewGroup.LayoutParams layoutParams = v.getLayoutParams();
            layoutParams.height = getStatusBarHeight(v.getContext());
