...
public class InterceptorUtil {
    ...
    public static HttpLoggingInterceptor LogInterceptor() {
        return new HttpLoggingInterceptor(message -> Log.i(TAG, "retrofit: " + message)).setLevel(HttpLoggingInterceptor.Level.BODY);//设置打印数据的级别
    }
        ...
        return new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                if (bodyString != null) {
                    ...
                    HttpResponse bean = JSON.parseObject(bodyString, HttpResponse.class);
                    //假设当返回的code为42444时token失效
                    if (!bean.isSuccess()) {
                        //重新获取新token
                        //这用了一个特殊接口来获取新的Token
//                        Call<String> call = RetrofitManager.getInstence().service().loginByToken("123456", Token);
//                        //拿到请求体
